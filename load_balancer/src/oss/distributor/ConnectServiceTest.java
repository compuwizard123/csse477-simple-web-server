/*
 *****************************************************************************
 * $Id: ConnectServiceTest.java,v 1.2 2003/05/05 22:49:50 jheiss Exp $
 *****************************************************************************
...
class ConnectServiceTest implements Runnable
{
	...
	public void run()
	{
		...
		while (true)
		{
			List<List<Target>> targets = distributor.getTargets();
			...
			{
				Iterator<List<Target>> i = targets.iterator();
				while (i.hasNext())
				{
					try
					{
						Target target = i.next().get(0);
