/*****************************************************************************
 * $Id: Distributor.java,v 1.14 2003/07/15 18:30:47 jheiss Exp $
 *****************************************************************************
...
public class Distributor
{
	...
	private Distributor(String args[])
	{
		...
		try
		{
			...
			distributionAlgorithms = new ArrayList<DistributionAlgorithm>();
				...
				if (configNode.getNodeName().equals("algorithms"))
				{
					...
					for (int j=0 ; j<algosChildren.getLength() ; j++)
					{
						...
						if (algoNode.getNodeName().equals("algorithm"))
						{
							...
							DistributionAlgorithm distAlgo =
								(DistributionAlgorithm) constructObjectFromName(
			...
			//
			targetGroups = new ArrayList<List<Target>>();
			...
			if (targetGroups.size() > 0)
			{
				...
				Iterator<List<Target>> tgIter = targetGroups.iterator();
				int tgCounter = 0;
		...
		{
			System.err.println("Error reading config file1: " + e.getMessage());
			System.exit(1);
		}
		catch (SAXException e)
		{
			System.err.println("Error reading config file2: " + e.getMessage());
			System.exit(1);
		}
		catch (IOException e)
		{
			System.err.println("Error reading config file3: " + e.getMessage());
			System.exit(1);
		}
		catch (NumberFormatException e)
		{
			System.err.println("Error reading config file4: " + e.getMessage());
			System.exit(1);
		}
	...
	public List<DistributionAlgorithm> getDistributionAlgorithms()
	{
		return distributionAlgorithms;
	}

	public List<List<Target>> getTargetGroups()
	{
		return targetGroups;
	}
	 ...
	 */
	public List<List<Target>> getTargets()
	{
		List<List<Target>> all = new ArrayList<List<Target>>();

		Iterator<List<Target>> i = targetGroups.iterator();
		List<Target> tg;
		while (i.hasNext())
		{
			tg = i.next();
